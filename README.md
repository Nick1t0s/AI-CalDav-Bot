...
### 3.1. STT (голосовые сообщения)

Распознавание работает через OpenAI Whisper API и требует **отдельный** ключ OpenAI
(Основной `OPENAI_BASE_URL` часто указывает на провайдера без аудио-эндпоинта —
OpenRouter/vLLM):
...
1. Пользователь пишет фразу (или отправляет голосовое — оно распознаётся через STT).
